...
interface Todo {
  id: string;            // UUID (PK)
  userId: string;        // Cognito Identity ID (SK)
...
### 4.3 권한 제어 흐름 (Cognito Identity Pool)
...
1. AWS CDK 프로젝트 초기화
2. Cognito Identity Pool 설정 (비인증 접근 활성화)
3. DynamoDB 테이블 생성
4. Lambda 핸들러 구현 (CRUD + toggle)
5. API Gateway 설정 (IAM 인증 + CORS 구성)

### Phase 3: 프론트-백엔드 연동

1. Cognito Identity Pool 연동 (비인증 자격증명 발급)
2. API 클라이언트 구현 (`todoApi.ts` — SigV4 서명)
3. TodoContext를 API 호출 기반으로 전환 (로컬 스토리지는 캐시로 유지)
